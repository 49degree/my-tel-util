...
import com.guanri.android.ui.jpos.common.Notify;
import com.guanri.android.ui.jpos.services.GrPosService;
...
public class AidlRunService extends Service{
    ...
    public void onCreate(){
    	...
    	//初始化数据处理线程对象
    	try{
        	Notify.notify(AidlRunService.NOTIFY_ID,"POS服务通知","POS终端服务已经打开");
    	}catch(Exception e){
    		e.printStackTrace();
    	}
    	try{
    ...
    public void onDestroy(){
    	...
    	try{
    		Notify.clearNotify(NOTIFY_ID);//清除消息提示
    	}catch(Exception e){
    		e.printStackTrace();
    	}
    ...
    private final GrPosService.Stub mBinder = new GrPosService.Stub(){ 
        ...
        public boolean startPos(){
        	if(HAS_COMM_PORT){
        		...
        		Notify.notify(AidlRunService.NOTIFY_ID,"POS服务通知","POS终端未找到");
        	}
        	return true;
        }
        /**
        ...
        public boolean stopPos(){
        	if(posCenterTask!=null&&!posCenterTask.getTaskStop()){
        		...
        		posCenterTask = null;
        		Notify.notify(AidlRunService.NOTIFY_ID,"POS服务通知","POS终端已经关闭");
        	}
        	return true;
        }
    			...
    			try {
    				if (SerialPortAndroid.findAndroidDevice("/dev/ttyUSB0")) {
    					...
    					if(isAutoRun){
        					...
        					Notify.notify(AidlRunService.NOTIFY_ID,"POS服务通知","找到POS终端");
        					LOG_INFO="找到POS终端";
    					}
					}else{
						HAS_COMM_PORT = false;
						mBinder.stopPos();
						Notify.notify(AidlRunService.NOTIFY_ID,"POS服务通知","未找到POS终端");
						LOG_INFO="未找到POS终端";
    ...
    public static class PosCenterThread extends Thread{
    	...
    	public void run() {
    		...
    		if (!TerminalLink.GetConnected()) {
    			if(SerialPortAndroid.findAndroidDevice(TerminalLink.CommName)){
    				try{
    					...
    					Notify.notify(AidlRunService.NOTIFY_ID,"POS服务通知","设备连接成功,正在读取数据.....");
    				}catch(SecurityException se){
    		...
    		try {
    			...
    			Notify.notify(AidlRunService.NOTIFY_ID,"POS服务通知","POS连接已经关闭.....");
    		} catch (Exception e) {
    			e.printStackTrace();
    		}
