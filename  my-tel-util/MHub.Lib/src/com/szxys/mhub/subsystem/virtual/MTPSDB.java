...
public class MTPSDB {
	...
	public void save(MTPSEntity parameters) {
		String tempSql = "insert into mb_uonitoring_parameters(userID,appID,IsChange,DownTime)values("
				+ parameters.userID
				+ ","
				+ parameters.appID
				+ ","
	...
	public List<MTPSEntity> cachMTPSEntity(int IsChange) {
		...
		while (!cursor.isAfterLast()) {
			...
			parameters.userID = Integer.parseInt(cursor.getString(cursor
					.getColumnIndex("userID")));
			parameters.appID = Integer.parseInt(cursor.getString(cursor
					.getColumnIndex("appID")));
			int iDownTime = cursor.getColumnIndex("DownTime");
			parameters.downTime =cursor.getString(iDownTime);
			parameters.isChange = Integer.parseInt(cursor.getString(cursor
	...
	public void UpdateDownTimeByAppID(int IsChange, int appID, String downTime) {
		String tempSql = "UPDATE mb_uonitoring_parameters SET IsChange="
				+ IsChange + "," + "downTime=" + "'" + downTime + "'"
				+ "WHERE appID=" + appID;
		this.dbHelper.open(true);
		this.dbHelper.execSQL(tempSql);
	}
	...
	public void UpdateIsChangeByAppID(int appID) {
		String tempSql = "UPDATE mb_uonitoring_parameters SET IsChange=1 WHERE appID="
				+ appID;
		this.dbHelper.open(true);
		this.dbHelper.execSQL(tempSql);
	}
