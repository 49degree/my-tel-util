 ...
 *
 */
public class SaleQueryAcvitity extends ApsaiActivity{
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		setTitle(StringUtils.getStringFromValue(R.string.apsai_sale_query));
	...
	public void initArrayList(){
		// 批次
		from = new String[]{"BillNo","Insu_No","Plyh_name","Operate_time"};
		to = new int[]{R.id.apsai_sale_query_list_billno,R.id.apsai_sale_query_list_insu_no,R.id.apsai_sale_query_list_plyh_name,
				R.id.apsai_sale_query_list_operate_time};
		queryedRecordList.setHorizontalScrollBarEnabled(true);//设置可滚动
		queryedRecordList.setDividerHeight(0);//设置LIST没有分割线
		LayoutInflater mInflater = LayoutInflater.from(this);
		LinearLayout header = (LinearLayout)mInflater.inflate(R.layout.sale_query_list_header, null);
		queryedRecordList.addHeaderView(header);//设置表头
		queryedRecordList.setOnItemClickListener(new SaleDateItemClickListener());
	}
	...
	private ApsaiPageNumOperator apsaiPageNumOperator = new ApsaiPageNumOperator(){
		...
		public int queryAllRows(){
			return dbOperator.queryRowNum(DBBean.TB_SALE_ORDER, null);
		}
		/**
		 * 设置表格数据 
		 * 	pageInfo[0]//第几页
		...
		public void initListAdapter(int[] pageInfo){
			...
			drinkandurinesMapList = dbOperator.queryMapList(DBBean.TB_SALE_ORDER, from, param);	
			recordAdapter = new SaleCheckListAdapter(SaleQueryAcvitity.this,drinkandurinesMapList,
					R.layout.sale_query_list_item,from, to);
	...
	private class SaleDateItemClickListener implements OnItemClickListener{
		...
		public void onItemClick(AdapterView<?> parent, View view, int position,
			...
			// 插入了一列表 position需要减1
			if(position>0){
				Map<String,String> map = drinkandurinesMapList.get(position-1);
				String BillNo = map.get("BillNo");
				String Insu_No = map.get("Insu_No");
				InsuBackDialog insuBackDialog = new InsuBackDialog(SaleQueryAcvitity.this,BillNo,Insu_No);
				insuBackDialog.displayDlg();
			}
		}
