...
public class AMRNBPacketizer extends AbstractPacketizer implements Runnable {
	static Logger logger = Logger.getLogger(AMRNBPacketizer.class);
	...
	public void run() {
		...
		try {
			...
			if (header[5] != '\n') {
				logger.error("Bad header ! AMR not correcty supported by the phone !");
				return;
			}
		...
		logger.error("AMR packetizer stopped !");
