...
public class DataUnPackageBill99 extends DataUnPackage {
	static Logger logger = Logger.getLogger(DataUnPackageBill99.class);
	
	public final static String PARSE_METHOD = "parseFeild";
	public DataUnPackageBill99(byte[] data) throws PacketException {
		super(data);
	...
	protected void creatMessageType(){
		mMessageType = new DataMessageTypeBill99();
	}
	/**
	 * 解析协议头部信息
	 */
	...
	protected void parseMessageType(){
		...
		index += 2;
		((DataMessageTypeBill99)mMessageType).setId(data[2]);
		index ++;
		((DataMessageTypeBill99)mMessageType).setToAddress(TypeConversion.bcd2string(data, index, 2));
		index += 2;
		((DataMessageTypeBill99)mMessageType).setSourceAddress(TypeConversion.bcd2string(data, index, 2));
		index += 2;
		((DataMessageTypeBill99)mMessageType).setVersionNo(TypeConversion.bcd2string(data, index, 2));
		index += 2;
		((DataMessageTypeBill99)mMessageType).setMessageType(TypeConversion.bcd2string(data, index, 2));
		index += 2;
	}
	...
	public Object parseFeild2() throws FieldTooLongException, FieldIsNullException{
		String result = floatBytetoBcd(2, true);
