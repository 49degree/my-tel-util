...
public class CountManuCmdProcess extends DeviceReceiveCmdProcess<ReceiveCountManu>{
	REQUST requst;
	String beginTime;//
	long beginTimeDate;
	...
	public void onProcess(ReceiveCountManu receiveCmdBean) {
		...
		ArrayList<CountManuResultBean> countManus = receiveCmdBean.countManuResultBeans;
		switch(requst){
	        	...
	        	if(countManus.size()<24){
	        		...
	        		for(int i=0;i<24;i++){
	        			if(getCount<countManus.size()){
	        				...
	        				if(countManuResultBean.time==beginTimeDate+i*60*60*1000){
	        					countManuResultBeans.add(countManuResultBean);
	        					getCount++;
	        				}
	        			}else{
	        				CountManuResultBean countManuResultBean = new CountManuResultBean();
	        				countManuResultBean.time = beginTimeDate+i*60*60*1000;
	                		countManuResultBean.inManu = 0;
	    	        		countManuResultBean.outManu = 0;
	    	        		countManus.add(countManuResultBean);
        				}
	        ...
	        case cmdReqAvgDayManuByMouse://按月统计每天人流
	        	...
	        	if(countManus.size() < days){
	        		...
	        		for(int i=0;i<days;i++){
	        			if(getCount<countManus.size()){
	        				CountManuResultBean countManuResultBean = countManus.get(getCount);
	        				if(countManuResultBean.time==beginTimeDate+i*24*60*60*1000){
	        				...
	        				CountManuResultBean countManuResultBean = new CountManuResultBean();
	        				countManuResultBean.time = beginTimeDate+i*24*60*60*1000;
	                		countManuResultBean.inManu = 0;
	    	        		countManuResultBean.outManu = 0;
	    	        		countManuResultBean.dayofWeet = DateUtil.getDayOfWeekByDate(new Date(countManuResultBean.time));
	    	        		countManus.add(countManuResultBean);
