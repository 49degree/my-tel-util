...
import com.yangxp.rtcp.SenderReport;
...
public class RtpSocket implements Runnable {
	static Logger logger = Logger.getLogger(RtpSocket.class);
	...
	public void setDestination(InetAddress dest, int dport, int rtcpPort) {
		logger.debug("setDestination:"+dest+";port:"+dport+";rtcpPort:"+rtcpPort);
		mPort = dport;
		for (int i=0;i<mBufferCount;i++) {
			...
			while (mBufferCommitted.tryAcquire(4,TimeUnit.SECONDS)) {
				if (mOldTimestamp != 0) {
					...
					if ((mTimestamps[mBufferOut]-mOldTimestamp)>0) {
						...
						logger.error("TS: "+mTimestamps[mBufferOut]+" OLD: "+mOldTimestamp);
					}
				...
				//Log.e(TAG, "mSocket.send:"+mBufferOut);
	...
	protected static class AverageBitrate {
		...
		public void reset() {
			...
			mNow = TimeUtils.elapsedRealtime();
			mOldNow = mNow;
			mCount = 0;
			mDelta = 0;
			mTotal = 0;
