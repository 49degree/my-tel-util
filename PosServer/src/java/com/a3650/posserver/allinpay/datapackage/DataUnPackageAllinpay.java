/***********************************************************************
 * Module:  DataUnPackageBill99.java
 * Author:  Administrator
 * Purpose: Defines the Class DataUnPackageBill99
 ***********************************************************************/
...
public class DataUnPackageAllinpay extends DataUnPackage {
	static Logger logger = Logger.getLogger(DataUnPackageAllinpay.class);	
	public final static String PARSE_METHOD = "parseFeild";
	
	public DataUnPackageAllinpay(byte[] data) throws PacketException{
		super(data);
	}
	...
	protected void creatMessageType(){
		mMessageType = new DataMessageTypeAllinpay();
	}
	/**
	 * 解析协议头部信息
	 */
	...
	protected void parseMessageType(){
		mMessageType.setDataLength(TypeConversion.bytesToShortEx(data,0));
		index += 2;
		((DataMessageTypeAllinpay)mMessageType).setId(data[2]);
		index +=1;
		((DataMessageTypeAllinpay)mMessageType).setToAddress(TypeConversion.bcd2string(data, index, 2));
		index += 2;
		((DataMessageTypeAllinpay)mMessageType).setSourceAddress(TypeConversion.bcd2string(data, index, 2));
		index += 2;
		((DataMessageTypeAllinpay)mMessageType).setAppType(TypeConversion.bcd2string(data, index, 1));
		index += 1;
		((DataMessageTypeAllinpay)mMessageType).setTerminalCriterion(TypeConversion.bcd2string(data, index, 1));
		index += 1;
		String state_request = TypeConversion.bcd2string(data, index, 1);
		index += 1;
		((DataMessageTypeAllinpay)mMessageType).setTerminalState(state_request.substring(0,1));
		((DataMessageTypeAllinpay)mMessageType).setRequest(state_request.substring(1));

		index += 3;//保留使用N6
		//15位商户号+8位终端号 23字节
		String msgMerchantterminaID = TypeConversion.asciiToString(data, index, 23);
		index += 23;
		((DataMessageTypeAllinpay)mMessageType).setCompanyId(msgMerchantterminaID.substring(0,15));
		((DataMessageTypeAllinpay)mMessageType).setPosId(msgMerchantterminaID.substring(15));
		((DataMessageTypeAllinpay)mMessageType).setMessageType(TypeConversion.bcd2string(data, index, 2));
		index += 2;
	}
	 ...
	 * @throws FieldIsNullException
	 */
	public Object parseFeild7() throws FieldTooLongException, FieldIsNullException{
		
		String result = null;
		try {
			//result = asciiToString(10);
			result = fixBcdToIntL(10);
		} catch (UnsupportedEncodingException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
			return null;
	 ...
	 */
	public Object parseFeild15() throws FieldTooLongException, FieldIsNullException{
		String result = null;
		try {
			result = fixBcdToIntL(4);
		} catch (UnsupportedEncodingException e) {
	 ...
	 * @throws FieldTooLongException
	 */
	public Object parseFeild26() throws FieldTooLongException, FieldIsNullException{
		String result = null;
		try {
			result = fixBcdToIntL(2);
		} catch (UnsupportedEncodingException e) {
	 ...
	 * @throws FieldIsNullException
	 */
	public Object parseFeild32() throws FieldTooLongException, FieldIsNullException{
		
		String result = floatBytetoBcd(2, false);
		return (Object)result;
	}
	 ...
	 * @throws FieldIsNullException
	 */
	public Object parseFeild33() throws FieldTooLongException, FieldIsNullException{
	...
	public Object parseFeild46() throws FieldTooLongException, FieldIsNullException{
		int len  = Integer.parseInt(TypeConversion.bcd2string(data,index,2));//计算数据长度
		index += 2;
		byte[] mac = new byte[len];
	    System.arraycopy(data, index,mac,0, len);
	    index += len;
		return mac;
	}
	...
	public Object parseFeild48() throws FieldTooLongException, FieldIsNullException{
		...
		String result = floatBytetoBcd(3, true);
	...
	public Object parseFeild52() throws FieldTooLongException, FieldIsNullException{
		...
		byte[] pin = new byte[8];
		System.arraycopy(data,index,pin,0,8);
		index+=8;
		return pin;
	}
	...
	public Object parseFeild53() throws FieldTooLongException, FieldIsNullException{
		...
		try {
			...
			String temp = TypeConversion.bcd2string(data,index,8);
			index+=8;
			return temp;
			//fixLengthStr2cbcd(acount,16, false);
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
			return null;
	...
	public Object parseFeild54() throws FieldTooLongException, FieldIsNullException{
		...
		//String result = floatbytetointL(3);
		return (Object)result;
	}
	...
	public Object parseFeild60() throws FieldTooLongException, FieldIsNullException{
		//获取该域的全部数据
		//获取该域的全部数据
		TreeMap<Integer,DataSelfFieldLeaf> tlvData = new TreeMap<Integer,DataSelfFieldLeaf>();
		int tlvLength = Integer.parseInt(TypeConversion.bcd2string(data,index,2));//计算数据长度
		index +=2;
		
		logger.error("tlvLength60:"+tlvLength);
		byte[] tlvBuffer = new byte[(tlvLength+1)/2];
		System.arraycopy(data, index, tlvBuffer, 0, tlvBuffer.length);//获取数据
		String tlvBufferStr = TypeConversion.bcd2string(tlvBuffer);
		logger.error("tlvLength60:"+tlvBufferStr);
		index +=tlvBuffer.length;
		//解析数据
		int tvfIndex = 0;
		int dataIndex = 0;
		while(tvfIndex<tlvLength){
			try{
				dataIndex++;
				DataSelfFieldLeaf leaf = new DataSelfFieldLeaf();
				if(dataIndex==1){
					leaf.setTag("1");
					leaf.setValue(tlvBufferStr.substring(tvfIndex,2));
					tvfIndex += 2;
				}else if(dataIndex==2){
					leaf.setTag("2");
					leaf.setValue(tlvBufferStr.substring(tvfIndex,8));
					tvfIndex += 6;
				}else if(dataIndex==3){
					leaf.setTag("3");
					leaf.setValue(tlvBufferStr.substring(tvfIndex,11));
					tvfIndex += 3;
				}else if(dataIndex==4){
					leaf.setTag("4");
					leaf.setValue(tlvBufferStr.substring(tvfIndex,12));
					tvfIndex += 1;
				}else if(dataIndex==5){
					leaf.setTag("5");
					leaf.setValue(tlvBufferStr.substring(tvfIndex,13));
					tvfIndex += 1;
				}else if(dataIndex==6){
					leaf.setTag("6");
					leaf.setValue(tlvBufferStr.substring(tvfIndex,17));
					tvfIndex += 4;
				}else if(dataIndex==7){
					leaf.setTag("7");
					leaf.setValue(tlvBufferStr.substring(tvfIndex,19));
					tvfIndex += 2;
				}else
					break;
	 ...
	 * 位图位置 61
	 * 格式 变长 LLLVAR
	 * 类型:N...029(LLLVAR)，3个字节的长度值＋最大29个字节的数字字符域，
	 * ——	数据元长度					N3
	 * ——	61.1 原始交易批次号			N6
	 * ——	61.2 原始交易POS流水号		    N6
	 * ——	61.3 原始交易日期			    N4
		...
		while(tvfIndex<tlvLength){
			try{
				...
				if(dataIndex==1){
					...
					leaf.setTag("5");
					leaf.setValue(TypeConversion.asciiToString(tlvBuffer,tvfIndex,11));
					tvfIndex += 11;
				}else
					break;
				tlvData.put(dataIndex, leaf);
	 ...
	 * 描述: 原始信息域
	 * 1.28.34.2　用法一：终端密钥(POS Security Key)
	 * @param o
	 * @return
	 * @throws FieldTooLongException
	 */		
	public Object parseFeild62() throws FieldTooLongException, FieldIsNullException{
		...
		int tlvLength =  Integer.parseInt(TypeConversion.bcd2string(data,index,2));//计算数据长度
		logger.debug("index: "+ index +"tlvLength : " + tlvLength);
		index +=2;
		byte[] mac = new byte[tlvLength];
		System.arraycopy(data, index,mac,0, tlvLength);
		index +=tlvLength;
		return mac;
	...
	public Object parseFeild63() throws FieldTooLongException, FieldIsNullException{
		...
		int tvfIndex = 0;
		int dataIndex = 0;
		while(tvfIndex<tlvLength-2){//tlvLength-2防止取数据buffer太短
			try{
				dataIndex++;
				DataSelfFieldLeaf leaf = new DataSelfFieldLeaf();
				if(dataIndex==1){
					...
					leaf.setTag("2");
					leaf.setValue(TypeConversion.asciiToString(tlvBuffer, tvfIndex,tlvLength-tvfIndex));
				}else{
					break;
				}
				tlvData.put(dataIndex, leaf);
