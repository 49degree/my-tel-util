...
import com.yangxp.rtsp.utils.TimeUtils;
import com.yangxp.stream.Stream;
...
public class AACADTSPacketizer extends AbstractPacketizer implements Runnable {
	static Logger logger = Logger.getLogger(AACADTSPacketizer.class);
	...
	public void run() {
		...
		logger.debug("AAC ADTS packetizer started !");
		...
		long oldtime = TimeUtils.elapsedRealtime(), now = oldtime;
		...
		try {
			while (!Thread.interrupted()) {
				...
				samplingRate = Stream.AUDIO_SAMPLING_RATES[(header[2]&0x3C) >> 2];
			...
			logger.error("ArrayIndexOutOfBoundsException: "+(e.getMessage()!=null?e.getMessage():"unknown error"));
			e.printStackTrace();
		} catch (InterruptedException ignore) {}

		logger.debug("AAC ADTS packetizer stopped !");
