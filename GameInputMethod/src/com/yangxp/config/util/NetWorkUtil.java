package com.yangxp.config.util;
...
public class NetWorkUtil {
	...
	public final static int NETWORK_CONNECT_RESULE = 500002;// 网络连接结果通知

	/**
	 * 广播行为：指明连接状态改变，通过监听该动作能得知建立连接的结果，以及获得程序运行过程中网络的一些变化情况。 附带有网络状态信息
	 * 
	 * @see EXTRA_STATUS_INFO
	 */
	public final static String ACTION_CONNECTION_STATUS_CHANGE = "uih.mh.ACTION_CONNECTION_STATUS_CHANGE";
	 ...
	 * 获取当前网络连接类型
	 * 
	 * @return 当前网络连接类型；可能为 {@link #NET_TYPE_NONE}, {@link #NET_TYPE_WIFI}, 或者
	 *         {@link #NET_TYPE_GPRS}。
	 */
	public int getNetWorkInfo() {
		...
		Log.e(TAG, connec.getAllNetworkInfo().length+"");
	 ...
	 * 
	 * @param isDeleteAll
	 *            如果为true，则删除所有可用连接；否则只删除程序创建的连接
	...
	private class ConnectionChangeReciver extends BroadcastReceiver {
		...
		public void onReceive(Context context, Intent intent) {
			...
			if (action.equals(ConnectivityManager.CONNECTIVITY_ACTION)) {
				...
				if (mHandler != null) {
					Message msg = mHandler.obtainMessage(NETWORK_CONNECT_RESULE, new Boolean(mIsConnected));
		...
		public void run() {
			...
			if (activeNetInfo == null || !activeNetInfo.isAvailable()) {
				...
				if (mIsAuto) {
					...
					LogCat.v(TAG, "连接超时，无法建立网络连接");
					Intent tmpIntent = new Intent(ACTION_CONNECTION_STATUS_CHANGE);
					tmpIntent.putExtra(EXTRA_STATUS_INFO, STATUS_CONNECT_NO);
					mContext.sendBroadcast(tmpIntent);

					if (mHandler != null) {
						Message msg = mHandler.obtainMessage(NETWORK_CONNECT_RESULE, new Boolean(false));
						mHandler.sendMessage(msg);
					}
				}
			}
