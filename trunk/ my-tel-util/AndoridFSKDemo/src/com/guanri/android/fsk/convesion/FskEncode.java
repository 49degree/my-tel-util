...
import com.guanri.android.fsk.convesion.FskEnCodeResult.ParseValueImp;
...
public class FskEncode {
	...
	public static void main(String[] args){
		...
		FskCodeParams fskCodeParams = new FskCodeParams(2200,1200,110250,2,1200);
		
		FskEncode fskEncode = new FskEncode(fskCodeParams);
		
		
		byte[] s = "hhhhhhhhhh".getBytes();
		byte[] bu = new byte[2+s.length];
		for(int i=0;i<2;i++){
			bu[i] = -1;
		}
		System.arraycopy(s, 0, bu, 2, s.length);
