...
class CommCheckSum implements IBthDataCheckSum
{
	/**
	 ...
	 */
	@Override
	public byte SumCheck(byte[] aBuffer, int aStartPos,int aEndPos) {
		byte sum_check = 0;
		for (int i = aStartPos; i < aEndPos; i++) {
			sum_check  ^=aBuffer[i] & 0xff;
		}
...
//	 */
//	@Override
//	public byte crc(byte[] aBuffer,int aCheckedLength) {
//		byte crc = 0x00;
//		int i = 0;
//		while (aCheckedLength-- > 0) {
//			crc = crc8_table[0xff&crc ^ 0xff & aBuffer[i++]];
//		}
//		return crc;
//	}
	@Override
	public byte crc(byte[] aBuffer, int aStartPos, int aEndPos)
	{
		byte crc = 0x00;
		int i = aStartPos;
		int length = aEndPos - aStartPos;
		while (length-- > 0) {
			crc = crc8_table[0xff&crc ^ 0xff & aBuffer[i++]];
		}
		return crc;
	...
	private final static byte[] crc8_table = new byte[] { (byte) 0x00, (byte) 0x07, (byte) 0x0E,
