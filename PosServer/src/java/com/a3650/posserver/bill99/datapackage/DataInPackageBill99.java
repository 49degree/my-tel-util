...
public class DataInPackageBill99 extends DataInPackage {
	static Logger logger = Logger.getLogger(DataInPackageBill99.class);	
	public final static String PARSE_METHOD = "parseFeild";
	public DataInPackageBill99(TreeMap<Integer, Object> sendMap,
	...
	public byte[] parseFeild46(Object o) throws FieldTooLongException{
		...
		ArrayList<DataSelfFieldLeaf> data = (ArrayList<DataSelfFieldLeaf>)o;
		byte[] tlv = new byte[128];
		int index = 0;
		for(DataSelfFieldLeaf leaf:data){
			try{
	...
	public byte[] getMabSource(){
		...
		try {
			if(mDataMap.containsKey(2)){
			    value = mDataMap.get(2);
				temp = this.parseFeild2(value);
				macSource = Utils.insertEnoughLengthBuffer(macSource, index, temp, 1, temp.length-1, 128);
				index += temp.length-1;
			}

			if(mDataMap.containsKey(3)){
			//3  Processing Code 
				value =  mDataMap.get(3);
				temp = this.parseFeild3(value);//定长
				macSource = Utils.insertEnoughLengthBuffer(macSource, index, temp, 0, temp.length, 128);
				index += temp.length;
			}
			if(mDataMap.containsKey(4)){
				value =  mDataMap.get(4);
				temp = this.parseFeild4(value);//定长
				macSource = Utils.insertEnoughLengthBuffer(macSource, index, temp, 0, temp.length, 128);
				index += temp.length;
			}
			if(mDataMap.containsKey(11)){
				value =  mDataMap.get(11);
				temp = this.parseFeild11(value);//定长
				macSource = Utils.insertEnoughLengthBuffer(macSource, index, temp, 0, temp.length, 128);
				index += temp.length;
			}
			if(mDataMap.containsKey(12)){
				value =  mDataMap.get(12);
				temp = this.parseFeild12(value);//定长
				macSource = Utils.insertEnoughLengthBuffer(macSource, index, temp, 0, temp.length, 128);
				index += temp.length;
			}
			if(mDataMap.containsKey(13)){
				value =  mDataMap.get(13);
				temp = this.parseFeild13(value);//定长
				macSource = Utils.insertEnoughLengthBuffer(macSource, index, temp, 0, temp.length, 128);
				index += temp.length;
			}
			if(mDataMap.containsKey(49)){
				value =  mDataMap.get(49);
				temp = this.parseFeild49(value);//定长
				macSource = Utils.insertEnoughLengthBuffer(macSource, index, temp, 0, temp.length, 128);
				index += temp.length;
			}
			if(mDataMap.containsKey(38)){
				value =  mDataMap.get(38);
				temp = this.parseFeild38(value);//定长
				macSource = Utils.insertEnoughLengthBuffer(macSource, index, temp, 0, temp.length, 128);
				index += temp.length;
			}
			if(mDataMap.containsKey(39)){
				value =  mDataMap.get(39);
				temp = this.parseFeild39(value);//定长
				macSource = Utils.insertEnoughLengthBuffer(macSource, index, temp, 0, temp.length, 128);
				index += temp.length;
			}
			if(mDataMap.containsKey(41)){
				value =  mDataMap.get(41);
